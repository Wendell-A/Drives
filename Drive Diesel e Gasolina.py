...
MELHORIA (sem mudar demais regras):
- Etapa 1 agora também respeita a data_de_carregamento (anti "viagem no tempo"):
  só aceita match se match['data'] >= data_de_carregamento (quando data_de_carregamento existir).
- Garantia de "mais recente" nas descargas:
  antes de drop_duplicates(keep='last'), ordena por data (e remove NaT) para que o "last" seja o mais novo de fato.
  (aplicado na Etapa 1 e Etapa 2)
"""
...
class Config:
    ...
    RELATORIO_DIVERGENCIA_CONFIG: Dict[str, Any] = {
        ...
        "file_name": "Relatório de divergência Gasolina e Diesel.xlsx"
    }
...
class SharePointClient:
    ...
    # --- MÉTODOS ROBUSTOS DE ATUALIZAÇÃO DE ABA ---
    ...
    def upload_and_overwrite_excel(self, df_to_write: pd.DataFrame):
        """Método legado: Cria arquivo do zero (apenas se não existir)."""
    ...
    def update_specific_sheet(self, df_to_write: pd.DataFrame, sheet_name_target: str = 'Divergencias'):
        """
        Verifica se o arquivo existe, limpa a aba alvo (preservando outras) e escreve os dados
        usando um range calculado dinamicamente para evitar erros de API.
        """
        config_relatorio = self.config.RELATORIO_DIVERGENCIA_CONFIG
        file_name = config_relatorio['file_name']
        ...
        try:
            ...
            if sheet_name_target in existing_sheets:
                # Se existe, limpa o conteúdo (mantendo formatação)
            ...
            # 4. Escrever dados com Range Calculado
...
def processar_dados_descargas(df: pd.DataFrame, config: Config) -> pd.DataFrame:
    ...
    # --------------------------------------------------------------------------
    # NOVA ETAPA: DE-PARA DE PRODUTOS (NORMALIZAÇÃO)
    # --------------------------------------------------------------------------
    logging.info("Aplicando De-Para para normalizar nomes de produtos...")
    
    # Primeiro, padronizamos para minúsculo e removemos espaços extras
    ...
    mapa_produtos = {
        ...
        # Diesel S10
        "diesel b s10 comum": "diesel b s10",
        "diesel b s10 aditivado": "diesel b s10",
        "diesel b s10 aditivada": "diesel b s10", # Prevendo erro de concordância
        
        # Diesel S500
        "diesel b s500 comum": "diesel b s500",
        "diesel b s500 aditivado": "diesel b s500",
        "diesel b s500 aditivada": "diesel b s500"
    }
    
    # Aplica o replace. Se o valor não estiver no dicionário, mantém o original.
    df['produto'] = df['produto'].replace(mapa_produtos)
    # --------------------------------------------------------------------------
    ...
    # O filtro abaixo agora funcionará corretamente pois os nomes foram normalizados
    df = df[df['produto'].isin(cfg['products_to_include'])]
    
    data_limite = datetime.now() - timedelta(days=20)
    df['data'] = pd.to_datetime(df['data'], errors='coerce')
    df = df[df['data'].notna() & (df['data'] > data_limite)]
    
    # Excluir registros de devolução/cancelamento
...
def cruzar_e_atualizar_transportes(df_transportes: pd.DataFrame, df_descargas: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, int], Set[int]]:
    ...
    # --------------------------------------------------------------------------
    # MELHORIA: garantir que o "keep='last'" pegue o registro MAIS RECENTE por data
    # --------------------------------------------------------------------------
    descargas_com_indice['data'] = pd.to_datetime(descargas_com_indice['data'], errors='coerce')
    descargas_com_indice = descargas_com_indice.dropna(subset=['data'])
    # --------------------------------------------------------------------------

    ### ALTERAÇÃO 4: INÍCIO - Usar a nova chave_primaria para o cruzamento ###
    # MELHORIA: ordenar por data antes do drop_duplicates
    descargas_unicas_primaria = (
        descargas_com_indice
        .sort_values('data')
        .drop_duplicates(subset=['chave_primaria'], keep='last')
    )
    ...
    for index, transporte in transportes_atualizado.iterrows():
        ...
        if match:
            # ------------------------------------------------------------------
            # MELHORIA: aplicar condição de data_de_carregamento também na Etapa 1
            # Evita "chegada" antes do "carregamento" quando data_de_carregamento existir.
            # ------------------------------------------------------------------
            data_carreg = transporte.get('data_de_carregamento')
            if pd.notna(data_carreg) and pd.notna(match.get('data')):
                if match['data'] < data_carreg:
                    continue
            # ------------------------------------------------------------------
    ...
    # MELHORIA: ordenar por data antes do drop_duplicates
    descargas_unicas_secundaria = (
        descargas_com_indice
        .sort_values('data')
        .drop_duplicates(subset=['chave_secundaria_placa'], keep='last')
    )
...
def main():
    try:
        ...
        if not df_descargas.empty:
            ...
            if not df_relatorio.empty:
                # FILTRO 1: Gasolina OU Diesel (Case Insensitive e Regex)
                termo_busca = 'Gasolina|Diesel'
                filtro_produto = df_relatorio['produto'].astype(str).str.contains(termo_busca, case=False, na=False, regex=True)
                df_relatorio_final = df_relatorio[filtro_produto]
            
            if not df_relatorio_final.empty:
                # -----------------------------------------------------------
                # NOVO FILTRO: EXCLUIR EMPRESA "PHAENARETE"
                # -----------------------------------------------------------
                logging.info("Aplicando filtro para excluir empresa 'PHAENARETE'...")
                # Converte para string, remove espaços e coloca em maiúsculo para garantir a exclusão
                filtro_empresa = df_relatorio_final['empresa'].astype(str).str.strip().str.upper() != 'PHAENARETE'
                df_relatorio_final = df_relatorio_final[filtro_empresa]
                # -----------------------------------------------------------
            ...
            if not df_relatorio_final.empty:
                ...
                # USO DO NOVO MÉTODO PARA NÃO APAGAR OUTRAS ABAS
                ...
                df_vazio = pd.DataFrame(columns=[f"Nenhuma divergência de Gasolina ou Diesel encontrada na execução de {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}."])
