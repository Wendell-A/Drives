...
if __name__ == "__main__":
    try:
        logging.info("--- INICIANDO PROCESSO DE COLETA (SOMENTE SHAREPOINT) ---")
        Config.validate()
        ...
        if df_consolidado.empty: 
            ...
            # REMOVIDO: Função padronizar_datas que corrompia o formato.
            # As datas ficam exatamente como vieram da leitura (strings/texto).
            ...
            # Comparação direta de texto, sem converter
            mask_atuais = (
            ...
            if not df_aguardando.empty:
                # IMPORTANTE: dayfirst=True garante que 01/12 seja lido como 1 de Dezembro, e não 12 de Janeiro.
                # A conversão é feita APENAS na memória para o cálculo matemático, não altera a coluna original.
                dt_chegada = pd.to_datetime(
                    df_aguardando['data_chegada'], 
                    dayfirst=True,  # <--- CORREÇÃO AQUI
                    errors='coerce'
                )
                now = datetime.now()
            ...
            print("\n" + "*"*50)
            print("📈 DADOS CONSOLIDADOS E ENVIADOS AO SHAREPOINT!")
            print(f"Caminho: {Config.DESTINO_SHAREPOINT['folder_path']}/{Config.DESTINO_SHAREPOINT['file_name']}")
