...
def main():
    try:
        ...
        if not df_descargas.empty:
            ...
            if not df_relatorio.empty:
                # NOVO FILTRO: Anidro OU Biodiesel (Case Insensitive e Regex)
                termo_busca = 'Gasolina A|Diesel A S10|Diesel A S500|Diesel B S10|Diesel B S500'
            ...
            if not df_relatorio_final.empty:
                ...
                df_vazio = pd.DataFrame(columns=[f"Nenhuma divergência de Anidro ou Biodiesel encontrada na execução de {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}."])
